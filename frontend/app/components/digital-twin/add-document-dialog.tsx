...
import type { DocumentCategory, HomeDocumentCreate } from "@/lib/types";
import { DOCUMENT_CATEGORIES, DOCUMENT_CATEGORY_ORDER } from "@/lib/constants/document-categories";
...
/** No real file upload yet — object storage isn't wired up (see Phase 1 of the
 * Digital Twin plan). This captures the structured metadata a real upload
 * flow would extract/confirm, so the rest of the UI (list, badges, filters)
 * works end to end today and only the storage step needs to be swapped in later. */
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body: HomeDocumentCreate = {
        ...
        expiresAt: category === "warranty" && expiresAt ? new Date(expiresAt).toISOString() : null,
      };
