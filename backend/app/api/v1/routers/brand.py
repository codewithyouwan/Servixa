...
convention as crm.py for the Contractor CRM.

Security model: reads are open to any logged-in role (a brand profile is
meant to be browsed by homeowners and contractors, same as the plan's
"public-facing summary" language for Company Overview) — gated by
get_current_user, not a role check. Writes that edit the brand's own content
require require_brand. Submitting a support ticket is the one write open to
any logged-in role, since it's homeowners/contractors contacting the brand,
not the brand editing itself.
"""

from fastapi import APIRouter, Depends, HTTPException, Query, status

from app.dependencies.auth import get_current_user, require_brand
