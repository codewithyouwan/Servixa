...
Orders, Invoices. One router for the whole module, same convention as
dashboard.py for the homeowner side.
"""

from fastapi import APIRouter, Depends, HTTPException, Query, status

from app.dependencies.auth import require_service_provider
...
def dashboard(user: UserOut = Depends(require_service_provider)) -> ApiResponse[CrmDashboardOut]:
...
def customers(user: UserOut = Depends(require_service_provider)) -> ApiResponse[list[CustomerOut]]:
...
def leads(user: UserOut = Depends(require_service_provider)) -> ApiResponse[list[LeadOut]]:
...
def accept_lead(
    lead_id: str, user: UserOut = Depends(require_service_provider)
) -> ApiResponse[LeadOut]:
...
def quotes(user: UserOut = Depends(require_service_provider)) -> ApiResponse[list[CrmQuoteOut]]:
...
def create_quote(
    body: CrmQuoteCreate, user: UserOut = Depends(require_service_provider)
) -> ApiResponse[CrmQuoteOut]:
    return ApiResponse(data=crm_service.create_quote(body))


@router.post(
    "/quotes/ai-draft", response_model=ApiResponse[AiQuoteDraftOut], response_model_by_alias=True
)
def ai_quote_draft(
    body: AiQuoteDraftRequest, user: UserOut = Depends(require_service_provider)
...
def orders(user: UserOut = Depends(require_service_provider)) -> ApiResponse[list[OrderOut]]:
...
def invoices(user: UserOut = Depends(require_service_provider)) -> ApiResponse[list[InvoiceOut]]:
...
def mark_invoice_paid(
    invoice_id: str, user: UserOut = Depends(require_service_provider)
...
@router.get(
    "/documents", response_model=ApiResponse[list[CrmDocumentOut]], response_model_by_alias=True
)
def documents(
    category: CrmDocumentCategory | None = Query(default=None),
    user: UserOut = Depends(require_service_provider),
) -> ApiResponse[list[CrmDocumentOut]]:
    return ApiResponse(data=crm_service.list_documents(category))


@router.post(
    "/documents", response_model=ApiResponse[CrmDocumentOut], response_model_by_alias=True
)
def create_document(
    body: CrmDocumentCreate, user: UserOut = Depends(require_service_provider)
